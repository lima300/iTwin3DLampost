...
> {
  ...
  public getPoints(): Point3d[] {
    if (undefined === this.props.range) return [];

    return this.state.pointGenerator.generatePoints(
  ...
  private _onChangePointMode = (pointMode: PointMode) => {
    ...
    switch (pointMode) {
      case PointMode.Circle: {
        pointGenerator = new CirclePointGenerator();
        break;
      }
      case PointMode.Cross: {
        pointGenerator = new CrossPointGenerator();
        break;
      }
      default:
      case PointMode.Random: {
        pointGenerator = new RandomPointGenerator();
        break;
      }
    }
  ...
  public componentDidUpdate(
    prevProps: PointSelectorProps,
    prevState: PointSelectorState
  ) {
    if (
      undefined !== this.props.range &&
  ...
  public render() {
    return (
      <LabeledSelect
        label="Points"
        size="small"
        value={this.state.pointMode}
        onChange={this._onChangePointMode}
